...
3、实现Callable接口，重写call方法，FutureTask能够接收Callable类型的参数，用来处理有返回结果的情况
...
懒汉式只有在实际需要时才去创建实例，同时给静态方法加了一把锁，所有来获取实例的线程都去竞争这把锁，不管实例有没有创建。

这样实现的问题是锁的粒度太粗，实际上只有首次创建实例的时候，为了防止多线程同时创建多个实例才需要锁，当实例已经创建完成之后直接返回实例即可。
...
DCL模式也是在使用时才会创建，且只有在首次创建实例时有锁竞争，实例创建完成后，没次获取实例时，在第一次检测时已经知道实例已创建，不会再执行synchronized里的代码，没有锁的竞争，提高效率。

注意到这里使用volatile关键字来修饰instance，最关键的作用是防止指令重排序。因为Java中new一个对象在JVM中分3个步骤执行：

1. 在堆内存中开辟内存空间
2. 对Singleton进行实例化
3. 把变量instance指向堆内空间地址

由于重排序的存在，可能步骤3先于步骤2执行，加入在线程A获取锁执行完步骤3而尚未执行步骤2时，线程B来获取实例会发现instance不会空，于是直接返回instance，然而此时的实例尚未初始化，线程B获取到的是不完整的实例，在使用时可能会出问题。
...
<img src="面试题.assets/image-20220421170242072.png" alt="image-20220421170242072" style="zoom: 33%;" />
...
进程本质上是一个程序，是计算机分配资源的基本单位，一个进程至少拥有一个线程。

线程是进程中的一个任务，是CPU调度的基本单位
...
并发：在一段时间内，多个任务都会被处理，但在某一时刻只有一个任务在执行。单核处理器可以做到并发，当线程A和B之间切换速度足够快时，宏观上表现为在一段时间内能同时运行多个程序

并行：在同一时刻有多个任务在执行，需要多核处理器才能实现，是物理上的多个线程同时进行
...
请求和保持条件：进程因为请求资源而阻塞时，对已获得的资源保持不放

不剥夺条件：进程已获得的资源在未使用完之前，不能剥夺，只能在使用完时由自己释放

循环等待：在发生死锁时，存在一个进程-资源的环形链

##### 2）预防死锁

资源一次性分配

只要有一个资源得不到分配，也不给这个进程分配其它资源
...
- 抽象类中可以有静态代码块和静态方法，接口中不能有（jdk1.8接口中可以有静态方法）
...
**第二范式：**在满足第一范式的基础上，要求非主键属性完全依赖于主键，不能存在仅依赖主关键字一部分的属性（针对联合主键），或者说不能有任何一列与主键没有关系。例如以学号为主键时，学生表的其他字段都完全依赖学号，不能出现诸如“课程名”等不依赖学号的设计
...
总：多版本并发控制（MVCC）：同一条记录在系统中可以存在多个版本
...
delete：根据条件删除表中满足条件的数据，如果不指定where子句将删除表中所有记录，可以回滚

drop：删除表结构和表数据，不能回滚，不会触发触发器

truncate：清空表，重置自增主键的值，只能作用于表，不能回滚，不会触发触发器
...
<img src="面试题.assets/image-20220418160445766.png" alt="image-20220418160445766" style="zoom: 50%;" />
...
- 1xx：中间状态，不常见
  ...
  - 101：服务端理解客户端的请求，将切换不同的协议完成这个请求，只有在切换新的协议更有好处的时候才应该采取类似措施
- 2xx：服务器成功处理客户端的请求
  - 200：请求成功，请求所希望的响应头或数据体将随此响应返回
  - 204：服务器成功处理了请求，但不需要返回任何实体内容（响应头没有body数据）
- 3xx：客户端请求的资源发生了变动，需要客户端用新的URL重新请求，即重定向
  - 301：永久重定向
  - 302：临时重定向
- 4xx：客户端错误，服务器无法处理
  - 400：错误请求，语义有误，当前请求无法被服务器理解或请求参数有误
  - 401：当前请求需要用户验证
  - 403：服务器禁止访问资源
...
<img src="面试题.assets/image-20220403180143457.png" alt="image-20220403180143457" style="zoom:33%;" />
...
<img src="面试题.assets/image-20220226154851196.png" alt="image-20220226154851196" style="zoom: 50%;" />
...
<img src="面试题.assets/image-20220421103038218.png" alt="image-20220421103038218" style="zoom: 33%;" />

Header（头部）：一个JSON对象（最后将该对象使用Base64URL算法转成字符串）

- alg：表示签名的算法，默认是HMAC SHA256
- typ：表示这个令牌（token）的类型，JWT令牌统一写成JWT

Payload（负载）：一个JSON对象（最后将该对象使用Base64URL算法转成字符串），用来存放实际需要传递的数据。JWT规定了7个官方字段供选用，可以自定义字段。JWT默认是不加密的，任何人都可以读到，适合在网络中传输非敏感信息

Signature（签名）：对上面两部分数据签名，防止数据篡改。首先需要指定一个密钥，该密钥只有服务器知道，然后使用Header里指定的签名算法生成签名

<img src="面试题.assets/image-20220421104051180.png" alt="image-20220421104051180" style="zoom: 50%;" />
...
> 使用#{}而不是${}，可以很大程度防止SQL注入

- 因为#{}是参数占位符，MyBatis在处理#{}时将SQL中的#{}替换成?，调用PreparedStatement的set()方法赋值，也就是说已经定义好了参数，在组成SQL语句时只做填空。#{}可以有效防止SQL注入，提高系统的安全性
...
- Hash：Hash是一个键值对集合，适合于存储对象，内部有两种编码方式，Redis会根据数据量来自适应选择两种编码方式中较优的一种；在数据条目较少，数据值较小时Redis会采用压缩列表
...
<img src="面试题.assets/image-20220312144841218.png" alt="image-20220312144841218" style="zoom:33%;" />

~~~markdown
- 3种特殊的数据类型：
Bitmaps：位图，Bitmap可以看作一个以位为单位的数组，数组中每个单元只能存0或1，数组的下标在Bitmap中叫做偏移量
Hyperloglogs：HyperLogLog是用于统计基数的数据集合类型
GEO：主要用于存储地理位置信息
~~~

#### Redis快的原因

- 基于内存实现：数据本身在内存中，不需要从磁盘中读数据
- 高效的数据结构
- 合理的数据编码
- 合理的线程模型
  - I/O多路复用模型同时监听客户端连接，将事件推送到一个队列中然后逐个被执行
  - 单线程，在执行过程中不需要进行上下文切换
...
##### 1 最简单场景：用List类型实现消息队列

<img src="面试题.assets/image-20220312151127151.png" alt="image-20220312151127151" style="zoom: 50%;" />

<img src="面试题.assets/image-20220312151153464.png" alt="image-20220312151153464" style="zoom: 50%;" />

问题：当队列中没有信息，消费者继续执行RPOP时会返回nil，而我们在编写消费者逻辑时一般处理成死循环，达到不断从队列中拉取消息的效果，如果此时队列为空，会造成CPU空转，浪费资源，也会对Redis造成压力

解决：当队列为空时，设置消费者sleep，但会造成消费者信息处理延迟。进一步，Redis提供了阻塞式拉取消息的命令，BLPOP/BRPOP

<img src="面试题.assets/image-20220312151834113.png" alt="image-20220312151834113" style="zoom:50%;" />

缺点：

- 不支持重复消费：消费者拉取消息后，消息从List中删除，无法被其它消费者再次消费
- 消息丢失：消费者拉取消息后如果发生意外宕机，这条消息将永远丢失

##### 2 发布/订阅模型：Pub/Sub

Redis提供了PUBLISH/SUBSCRIBE命令完成发布、订阅操作

<img src="面试题.assets/image-20220312152636339.png" alt="image-20220312152636339" style="zoom:50%;" />

<img src="面试题.assets/image-20220312152655366.png" alt="image-20220312152655366" style="zoom:50%;" />

优势：支持多组生产者、消费者处理消息

缺点：丢数据，3种场景

- 消费者下线：在发布/订阅流程中没有任何数据的存储，消息均为实时转发。消费者必须先订阅消息队列，生产者才能发布消息，否则消息会丢失
- Redis宕机：Pub/Sub相关操作，不会写入RDB和AOF
- 消息堆积：每个消费者订阅一个队列，Redis都会在Server上为这个消费者分配一个缓冲区，当内容超过缓冲区配置的上限，Redis强制将该消费者踢出

##### 3 趋于成熟的队列：Stream

- 支持阻塞式拉取消息
- 支持发布/订阅模式
- 消息处理异常时，消息不丢失：当消费者处理完消息后，需要执行XACK命令告知Redis，Redis将这条消息标记为“处理完成”
- 消息持久化：Stream是新增加的数据类型，每个写操作也都会写入到RDB和AOF中
- 消息堆积：发布消息时可以指定队列的最大长度，当长度超过上限后，旧消息被删除

<img src="面试题.assets/image-20220312154455984.png" alt="image-20220312154455984" style="zoom:50%;" />

链接：https://mp.weixin.qq.com/s/aNdWL3xKtjOd_briXL6Bjg

#### Redis和数据库怎么保持一致性？

不考虑并发，正常情况下，无论谁先谁后都可以让二者保持一致

异常情况：

1先更新缓存，后更新数据库

​	缓存更新成功，数据库更新失败，请求命中缓存时拿到正确值，一旦缓存失效，就会从数据库中读到旧值

２先更新数据库

​	之后的读请求读到的都是旧值，只有当缓存失效后才能从数据库拿到正确的值

**解决方案：**

１．先更新数据库，后删除缓存，并引入消息队列（异步重试）

<img src="面试题.assets/image-20220312160747882.png" alt="image-20220312160747882" style="zoom: 25%;" />

2．订阅数据库变更日志，再操作缓存

<img src="面试题.assets/image-20220312161039853.png" alt="image-20220312161039853" style="zoom: 33%;" />

链接：https://mp.weixin.qq.com/s/RDOKLnG7P1j5Ehu3EyrsCQ

#### Redis 如何实现分布式锁的？

先使用setnx来争抢锁，获取锁后再用expire给锁加一个过期时间防止锁忘记释放

如果在setnx后执行expire之前进程意外crash或要重启维护，该怎么办？

set指令可以通过参数设置同时把setnx和expire合成一条指令
...
- [ ] 剑指 Offer 40. 最小的k个数（o(n)/o(nlogn)/o(nlogk)）

#### 堆排

- [x] 堆排序
- [x] lc215 数组中的第K个最大元素（1、堆排序，自己实现堆；2、使用优先队列实现堆，最大堆最小堆，堆只用K个最小堆；3、快排分割思路）
...
- [x] 剑指 Offer 68 - II. 二叉树的最近公共祖先（与-Ⅰ的区别，二叉树的递归一般都操作的是root节点）
- [x] 剑指 Offer 07. 重建二叉树（递归函数的参数设置，重建二叉树需要根节点，节点在数组中的左边界、右边界，由于有前序和中序两个节点，故设计4个参数）
- [x] 剑指 Offer 27. 二叉树的镜像（已做，习惯递归的思路，如果不递归单独交换二叉树左右子节点怎么做，递归起来又该怎么做）
- [x] lc98 验证二叉搜索树（利用二叉搜索树中序遍历的特征，只用判断当前值是否大于前一个值，递归/非递归）
- [x] **lc572 另一棵树的子树**（验证一棵树是不是另一棵的子树，本题的关键点在于对树相同和子树的分离，构造两个函数，掌握比较两棵树是否相同的基本操作）
- [x] **lc110 平衡二叉树**（1.直接递归，自顶向下，o(N^2)；2.自底向上，如果遇到子树不平衡的情况，提前返回）
- [ ] lc114 二叉树展开为链表
- [ ] lc331 验证二叉树的前序序列化
...
- [x] **lc128 最长连续序列**（1、这道题求出的实际上子集合，而不是子序列，注意与最长连续子序列区分开来，子序列是不允许修改数组中元素的顺序的；2、如果x的前序数存在于数组中，那么x可以不再考虑了，因为从x-1开始的子序列一定比它长，以此减少进入下层循环的数据量，使最终时间复杂度达到o(n)）
- [x] lc56 合并区间（再做，学习数组遍历时的处理，对于第一个元素直接填进去，有合并的情况直接修改区间的右边界）
- [x] **lc5 最长回文子串**（中心扩散，注意先分别向左和向右移动，处理字符相同的情况，注意临界条件）
- [x] lc76 最小覆盖子串（滑动窗口，难）
- [x] lc26 删除有序数组中的重复项
- [x] lc80 删除有序数组中的重复项 II
- [x] lc149 直线上最多的点数（3重循环，判断多点是否共线，避免计算斜率时分母为0的情况，用乘法代替除法）

#### 二分

- [x] **lc69 x 的平方根**（二分查找，为了防止算平方时溢出，用除法代替，注意除法要考虑0，1特殊情况）
- [x] lc704 二分查找（基本二分查找，注意数组只有一个元素的情况，while条件里应该是l<=r）

#### 前缀和

- [x] **lc560 和为 K 的子数组**（1、前缀和的思想，用map存储前缀和为x的个数，因为preSum - (preSum - k) = k，因此在计算包含当前数的前缀和之后，如果map中存在key为preSum-k的值就说明找到了新的子数组；2、理解为什么是get(preSum-k)，因为序列中可能出现多个0）
- [x] lc1248 统计「优美子数组」（与lc560前缀和思路一致，维护一个存储单元存储的是前缀中有x个奇数的情况有几种；以此题为例，初始时有0个奇数的组合有1种，即空数组，遍历数组，如果第一个数是奇数，且最终就是要找一个奇数的子数组，那么我们需要看当前前缀和为0（1-1）的有几种，因为1-（1-1）=1，如果有则加上前缀和为0的子数组个数，如果没有，则维护前缀和为1的数据）
- [x] lc930 和相同的二元子数组（前缀和恰好用来解决的是连续子数组的问题）

#### 回溯

- [x] 剑指 Offer 12. 矩阵中的路径（1、没有说必须从左上角开始匹配，所以要考虑一个二层遍历，每一个元素都可以作为起始值；2、判断退出递归的条件，不满足条件时直接剪枝；3、每个格子只能用一次，用过之后可以设置为空字符，不再使用额外空间，注意：用visited的数组作为额外空间记录每个格子是否使用也可行，已验证）
- [x] lc78 子集（递归与回溯）
- [x] lc90 子集 II（有重复元素的子集，剪枝条件，为什么是i>start）
- [x] lc46 全排列（思考为什么这里用used，不用begin？因为第一次可能选了第二个数，下一层可能选第一个数，如果用start就错过了前面没选择的数，用used结合i=0，每次都遍历整个数组但是用used控制还能不能选；为什么设置depth？要控制往最终结果集中添加元素的时机）
- [x] lc47 全排列 II（有重复数字，在lc46的基础上添加新的剪枝条件，当nums[i]==nums[i-1]且nums[i-1]未被使用的时候剪枝，如果不加nums[i-1]未被使用的条件，将没有结果）
- [x] lc39 组合总和（剪枝，会了）
- [x] lc40 组合总和 II（大小剪枝，同一层如果减去同一个数，后面分支得到的结果是前面分支的子集，因为可选择的数更少，因此可以剪掉；在剪枝时，也可以使用used数组区分出正确的情况和该剪枝的情况）
- [x] lc22 括号生成（深度优先遍历，不用回溯，每次用的是新字符串）

#### 链表

- [x] 剑指 Offer 52. 两个链表的第一个公共节点（双指针，走到null时换路）
- [x] 剑指 Offer 35. 复杂链表的复制（借助辅助的map空间，多练几遍，赋值时一定注意原链表和新链表的关系，不能将新链表的next指向原链表的元素）
- [x] lc83 删除排序链表中的重复元素（pre初始指向null，和cur，next移动）
- [x] lc82 删除排序链表中的重复元素 II（递归思路解决链表问题）
- [x] lc25 K个一组翻转链表（难，链表的反转作为基本操作；链表的操作，一般都要记录pre和next）
- [x] lc148 排序链表（掌握两个链表合并，找链表中点的基本操作）
...
- [x] lc200 岛屿数量（学习DFS解岛屿问题框架，从一块是陆地的格子开始dfs，将与之相连的陆地全部标记为另一种形态，表示这是同一岛屿，遍历结束后计数变量加1，后续再遇到新的陆地格，表示遇到另一个新岛屿）
- [x] lc695 岛屿的最大面积（由上题计数变为每次返回岛屿最大面积，取最大）

#### 贪心

- [x] BM95 分糖果（贪心的思路，正向遍历一遍，再逆向遍历一遍，注意逆向遍历时要防止值变得更小，要用Max判断）
- [x] BM96 主持人（能想到队列的思路，学习优先队列）
- [x] lc122 买卖股票的最佳时机Ⅱ（只要有收益就交易，每次交易的收益一定大于等于攒着不卖，只有股票价格一直是上涨的情况下等号成立）
- [x] lc45 跳跃游戏 II（贪心比动态规划快）
- [x] lc343 整数拆分（n大于等于4的情况都能归结到2和3相乘的情况，推导出应该要尽可能分解出最多的3，当n大于等于5时，分出3收益最大）
- [x] lc12 整数转罗马数字（难度不大，代码可以写的简洁）
- [x] lc452 用最少数量的箭引爆气球（区间，优化到可以只按区间右边界排序）
- [x] lc134 加油站

#### 动态规划

- [x] **剑指 Offer 42. 连续子数组的最大和**（dp数组表示当前下标之前的子数组最大和，如果前面的子数组和为负，说明不如不要前面的数，直接从当前数开始可能能取到最大，如果前面的子数组和为正，可以累加，即使当前数是负数也没关系，因为每轮计算dp后与当前最大值做比较，留下更大值）
- [x] 剑指 Offer 10- II. 青蛙跳台阶问题（注意取模）
- [x] 剑指 Offer 47. 礼物的最大价值（二维动态规划，已掌握）
- [x] lc64 最小路径和（与礼物最大价值一致，已掌握）
- [x] lc62 不同路径（同类题）
- [x] lc63 不同路径 II（路上带障碍，注意下标0，0点的方案数，会做）
- [x] lc983 最低票价（经典动态规划，但这道题与常规动态规划方向相反，对于当前天的消费，在3种选择中比较大小）
- [x] 摩尔斯电码解码（动态规划）
- [x] lc121 买卖股票的最佳时机（维护买入最低价和最大利润两个变量即可，或动态规划）
- [x] lc122 买卖股票的最佳时机Ⅱ（动态规划，二维dp）
- [x] lc45 跳跃游戏 II（动态规划，自己做的）
- [x] lc198 打家劫舍（二维动态规划，自己做的，可以优化成一维）
- [x] lc213 打家劫舍 II（考虑偷不偷第一家，比较两次大小）
- [x] lc91 解码方法（比摩尔斯解码边界更复杂，判断是否满足条件时可以把字符转成数字方便判断）
- [x] lc120 三角形最小路径和（二维dp变种，按常规做，会做）
- [x] lc152 乘积最大子数组（可以做二维dp，记录最大最小值，也可以一维dp，直接比较）
- [x] lc97 交错字符串（二维dp，学习如何装化成二维dp的，对于两个字符串合成另一个字符串，考虑二维）
- [x] lc53 最大子数组和（动态规划，注意dp数组中存的是当前下标的最大连续子数组和，可能不是最大，用新的变量存最大子数组和）
- [x] lc96 不同的二叉搜索树（当节点数量固定时，不管是1，2，3还是2，3，4，能构建的BST数量相同，当根节点确定时，BST的数量等于左子树的不同数量乘右子树的不同数量）
- [x] BM66 最长公共子串(需要注意这题要子串,与子序列区分开,也是动态规划)
- [x] lc42 接雨水（活用动态规划，先行一步，将当前元素左边最小值和右边最大值保留）

#### 双指针

- [x] 面试题 10.01. 合并排序的数组（把握排序数组的特征，利用好数组A后面预留的位置）
- [x] **lc15 三数之和**（medium，先排序的思路正确，注意这里双指针的思路是将双指针都放在当前数的右侧，这样去重更为方便，如果nums[i]和nums[i-1]值相同，那么以nums[i]以后的元素找到的结果一定包含在nums[i-1]里面，故可以以此为标准进行去重。如果把双指针摆在两侧，不好去重；另外，针对[-2,0,0,2,2]这种情况，当left对应0，right对应2时，也要去重）
- [x] lc18 四数之和（对比三数之和，四数之和多一层循环；另外注意这里四数之和target是一个参数，可以是负数，在第一层不能加nums[i]>target的过滤）
- [x] **lc16 最接近的三数之和**（采用与三数之和同样的思路，在判断条件上略有不同）
- [x] **lc3 无重复字符的最长子串**（找到重复字符上次出现的位置，要和原先的left比较大小取较大的，针对abba的情况，当left=2，right=3时，a已存在，直接取下标为0再加1等于1，而left现在在2位置，不能倒退回去）
- [x] **lc415 字符串相加**（大数相加，用两个指针分别指向字符串的结尾，设置循环只要其中一个字符串还有数字就循环，将已经出现空的字符串对应位置置为1，最后退出循环后根据进位值是否为1决定最前面是否有1，最终字符串翻转即为结果）
- [x] lc11 盛最多水的容器（双指针的移动过程，一个指针初始在头，一个指针初始在尾，如果左指针值小于右指针，右指针移动是无意义的，因为左指针决定了容器的高不可能超过当前值，右指针的内移只会导致宽度的进一步减小）
- [x] lc167 两数之和 II - 输入有序数组（左右指针，根据与target的大小关系控制移动，不难）
- [x] lc925 长按键入（easy，注意name先结束和typed先结束的情况即可）
- [x] lc125 验证回文串（easy，思路不难，要注意可以优化到一次遍历搞定，不是字母或数字就略过，不用先过滤再比较）
- [x] lc344 反转字符串（easy，写swap函数即可）
- [x] lc881 救生艇（medium，最重的和最轻的尝试组队）
- [x] lc977 有序数组的平方（easy，比较两边数的绝对值，绝对值大的放在新数组最后）
- [x] lc27 移除元素（easy，注意相向指针和快慢指针两种方式，相向指针会改变元素的相对位置，但可以达到最少的交换次数；快慢指针能保证元素的相对位置）

#### 场景题

- [x] 电影院选座（一开始的分类没有搞清楚，比较的是首部连续0，尾部连续0，和中间连续0的情况，取3者中最大）
- [ ] 实现LRU