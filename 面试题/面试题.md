...
- [x] lc215 数组中的第K个最大元素（堆排序）
- [x] lc704 二分查找（注意数组只有一个元素的情况，while条件里应该是l<=r）
- [x] lc128 最长连续序列（这道题求出的实际上子集合，而不是子序列，注意与最长连续子序列区分开来，子序列是不允许修改数组中元素的顺序的）
