...
- [x] 堆排序
- [x] lc215 数组中的第K个最大元素（堆排序，自己实现堆，使用优先队列实现堆，最大堆最小堆）
...
- [x] lc15 三数之和（先排序的思路正确，注意双指针的思路和找到一组符合条件后的处理）阿里
...
- [x] lc25 K 个一组翻转链表（难，链表的反转作为基本操作；链表的操作，一般都要记录pre和next）
...
- [x] 剑指 Offer 42. 连续子数组的最大和（常规动态规划，注意数据长度为1的情况）
